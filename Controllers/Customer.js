...
export const createCustomerDetails = async (req, res) => {
  try {
    ...
    if (!receiverNo) {
      ...
      if (lastCustomer && lastCustomer.receiverNo) {
        const lastNumber = parseInt(lastCustomer.receiverNo.split("-")[1]) || 999;
        finalReceiverNo = `R-${lastNumber + 1}`;
      } else {
        finalReceiverNo = "R-1000";
      }
    } else {
      // Check uniqueness of manual receiverNo
